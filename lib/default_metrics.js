...
function addChangingValues () {
  ...
  metrics._id = metrics.type + ':' + metrics.service + ':' + formatTimestamp(metrics.timestamp);
...
function formatTimestamp (date) {
   var yyyy = date.getFullYear().toString(),
       ...
       hh   = (date.getHours() + 1).toString(), // getHours() is zero-based
       mi   = date.getMinutes().toString(),
       ss   = date.getSeconds().toString(),
       mil  = date.getMilliseconds().toString();

   return yyyy + (mm[1]?mm:"0"+mm[0]) + (dd[1]?dd:"0"+dd[0]) + (mi[1]?mi:"0"+mi[0]) + (ss[1]?ss:"0"+ss[0]) + (mil[1]?(mil[2]?mil:"0"+mil[0]):"00"+mil[0]); // padding
}
